...
RegisterNetEvent("tuner:useAutodriveKit", function()
    ...
    lib.progressBar({
        duration = 5000,
        label = "Installing TuneX Autodrive Kit...",
        useWhileDead = false,
        canCancel = false,
    ...
    if vehicle then
        ...
        hasAutodriveKit = true
        TriggerServerEvent("tuner:applyAutodriveKit", plate)
        lib.notify({ title = "TuneX Autodrive", description = "Autodrive kit installed!", type = "success" })
    end
end)

RegisterNUICallback('removeAutoDriveKit', function(_, cb)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    ...
    if not installedKits[plate] then
        lib.notify({ title = "TuneX", description = "No AutoDrive Kit installed.", type = "error" })
        cb({ status = "error" })
        return
    end
    ...
    lib.progressBar({
        duration = 10000,
        label = 'Removing TuneX AutoDrive Kit',
        useWhileDead = false,
        canCancel = true,
    ...
    installedKits[plate] = nil
    hasAutodriveKit = false
    TriggerServerEvent('tuner:removeAutoDriveKit', plate)
    lib.notify({ title = "TuneX", description = "AutoDrive Kit removed.", type = "inform" })
...
CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if not installedKits[plate] then
                QBCore.Functions.TriggerCallback("tuner:hasAutodriveKit", function(hasKit)
                    if hasKit then
                        installedKits[plate] = true
                        hasAutodriveKit = true
                    end
