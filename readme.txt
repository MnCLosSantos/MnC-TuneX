...
This also adds a command "jobtune" thats locked to these jobs 
"police, ambulance, bcso" no chance of failure for command just needs to be used twice. 
The amount the car is made faster is able to be configd as well as a built in plate changer that comes deactivated.  
The base i used for this edit is linked below i created this as this wasnt working for qb-core as an item only as a command, 
so i decided to make it what it should really be. The new version is now relying on QBCORE and not standalone or esx compatible.
...
1) install items qb/qb-core/shared/items.lua

2) add images qb/qb-inventory/html/images
